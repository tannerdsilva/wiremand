			...
			do {
				try await scheduler.runSchedule(name: taskName, interval: .seconds(10)) {
					do {
						...
						let checkHandshakes = try await Command("sudo wg show \(String(self.interfaceName)) latest-handshakes").runSync()
						...
						let checkEndpoints = try await Command("sudo wg show \(self.interfaceName) endpoints").runSync()
