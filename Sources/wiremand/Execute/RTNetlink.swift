...
extension RTNetlink {
	public struct AddressRecord:Hashable, Codable {
		public struct AddressFlags: OptionSet, Hashable, Sendable, Codable {
			...
			public func encode(to encoder: Encoder) throws {
				...
				for (bit, name) in AddressFlags.flagMap {
					if self.rawValue & bit != 0 {
						flagNames.append(name)
					}
				}
				try container.encode(flagNames)
			}

			public init(from decoder: Decoder) throws {
				let container = try decoder.singleValueContainer()
				let flagNames = try container.decode([String].self)

				var rawVal = UInt32()
				for name in flagNames {
					if let bit = AddressFlags.flagMap.first(where: { $0.value == name })?.key {
						rawVal |= bit
					}
				}
				self = Self(rawValue: rawVal)
			}
		}
		...
		init(_ r:UnsafeMutablePointer<ifaddrmsg>, _ tb:UnsafeMutablePointer<UnsafeMutablePointer<rtattr>?>?) {
			...
			var flags: UInt32 = 0
			let flagsResult = get_attribute_uint32_ifa(tb, Int32(IFA_FLAGS), &flags)
			if flagsResult == 0 {
				self.flags = AddressFlags(rawValue: flags)
			} else {
				self.flags = AddressFlags(rawValue: 0)
			}
...
extension RTNetlink {
	public struct RouteRecord:Hashable, Codable {
		...
		init(_ r:UnsafeMutablePointer<rtmsg>, _ tb:UnsafeMutablePointer<UnsafeMutablePointer<rtattr>?>?) {
			...
			get_attribute_data_rt(r.pointee.rtm_family, tb, RTA_PREFSRC, &src)
			if (src != nil) {
				self.source = String(cString:src!)
				free(src)
			} else {
				self.source = nil
			}
