...
internal func readRandomData(size:size_t) throws -> [UInt8] {
	...
	return Array(newBuffer.bindMemory(to:UInt8.self))
}
...
internal class WireguardDatabaseV2 {
	...
	internal struct NetworkDatabase {
		private enum Databases:String {
			case subnetNameHash_subnetName = "addrdb:subnetNameHash_subnetName"
			case subnetNameHash_securityKey = "addrdb:subnetNameHash_securityKey"
			case subnetNameHash_networkV6 = "addrdb:subnetNameHash_networkV6"
			case networkV6_subnetNameHash = "addrdb:networkV6_subnetNameHash"
		}

		internal let subnetNameHash_subnetName:Database.Strict<SubnetNameHash, SubnetName>
		internal let subnetNameHash_securityKey:Database.Strict<SubnetNameHash, NetworkSecurityKey>
		
		internal let subnetNameHash_networkV6:Database.DupFixed<SubnetNameHash, NetworkV6>
		internal let networkV6_subnetNameHash:Database.DupFixed<NetworkV6, SubnetNameHash>
		...
		internal init(environment:Environment, tx:borrowing Transaction) throws {
			...
			subnetNameHash_subnetName = try Database.Strict<SubnetNameHash, SubnetName>(env:environment, name:Databases.subnetNameHash_subnetName.rawValue, flags:[.create], tx:subTrans)
			subnetNameHash_securityKey = try Database.Strict<SubnetNameHash, NetworkSecurityKey>(env:environment, name:Databases.subnetNameHash_securityKey.rawValue, flags:[.create], tx:subTrans)
			subnetNameHash_networkV6 = try Database.DupFixed<SubnetNameHash, NetworkV6>(env:environment, name:Databases.subnetNameHash_networkV6.rawValue, flags:[.create], tx:subTrans)
			networkV6_subnetNameHash = try Database.DupFixed<NetworkV6, SubnetNameHash>(env:environment, name:Databases.networkV6_subnetNameHash.rawValue, flags:[.create], tx:subTrans)
			try subTrans.commit()
		}
	}
	...
	init(path:URL) throws {
		...
		let makeEnvPath = path.appendingPathComponent("wgdb_v2")
		...
		// determine the current map size of the database
		let targetMaxMapSize:size_t = size_t(makeEnvPath.getFileSize()) + 1_000_000_000 // add one gigabyte to the current size

		let makeEnv:Environment
		do {
			makeEnv = try Environment(path:makeEnvPath.path, flags:mdb_flags, mapSize:targetMaxMapSize, maxReaders:64, maxDBs:32, mode:[.ownerReadWrite, .groupReadWrite, .otherRead])
		...
		self.metadata = try Database(env:makeEnv, name:"metadata", flags:[.create], tx:someTrans)
